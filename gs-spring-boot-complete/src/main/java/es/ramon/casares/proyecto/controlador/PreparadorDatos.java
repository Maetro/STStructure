...
public class PreparadorDatos {
    ...
    private void encode(
            final int s, final int c, final int word) {
    ...
    // Todos los objetos empiezan en posicion desconocida. Cuando desconocemos su posicion ocupa el espacio 0,0 de la
    // matriz
    private void inicializarPosiciones() throws IOException {
        ...
        writer = new BufferedWriter(new FileWriter(ficheroEscritura.getFile()));
    ...
    private void fillHashMap() throws IOException {
        ...
        while ((currentLine = datareader.readLine()) != null) {
            ...
            // System.out.println(Integer.valueOf(result[0]) + "->" + Integer.valueOf(result[0]) / FREQ_MUESTREO);

            final int instant = (int) Math.floor((Integer.valueOf(result[0]) - 1)
                    / this.configuracion.getSegundosEntreInstantes()); // En segundos
                ...
                if ((FILTRADO && (ID_FILTRADO == id)) || !FILTRADO) {
                    ...
                    if (instant != lastInstant) { // Cambiamos de instante
                        for (int i = lastInstant + 1; i < instant; i++) {
                            ...
                            // Acabamos de registrar todos los objetos de ese instante
                        }
                    ...
                    // if (FILTRADO) {
                    // System.out.println("---- "+ instant + " " + id + " " + y + " " + x);
                    // }
    ...
    private Point getNextPoint(
            final int id, final int instant) throws IOException {
        ...
        while ((currentLine = datareader.readLine()) != null) {
            cont++;
            final String[] result = currentLine.trim().split("\\s");
            final int ninstant = (int) Math.floor(Integer.valueOf(result[0])
                    / this.configuracion.getSegundosEntreInstantes()); // En segundos
        ...
        for (int i = 1; i <= this.numeroObjetos; i++) {
            if ((FILTRADO && (ID_FILTRADO == i)) || !FILTRADO) {
                ...
                if ((punto.getX() != -1) && (punto.getY() != -1)) {
                    ...
                    writer.write(lastInstant + " " + i + " " + punto.getY() + " " + punto.getX()
                    ...
                    // System.out.println(lastInstant + " " + i + " " + 0 + " " + 0 +
                    // System.getProperty("line.separator"));
                    // writer.write(lastInstant + " " + i + " " + 0 + " " + 0 + System.getProperty("line.separator"));
                    numObjetosDesconocidos++;
                    System.out.println(lastInstant + " " + i + " " + -1 + " " + -1
                            );
                    writer.write(lastInstant + " " + i + " " + -1 + " " + -1
                            + System.getProperty("line.separator"));
                }
            }
        }
                ...
                if ((objeto.getPoint().getX() != 0) && (objeto.getPoint().getY() != 0)) {
                    if ((instant - objeto.getPoint().getInstant()) > this.limiteSuperior) {
                        ...
                        if (!this.mapIdsDesaparecidos.get(i).equals("-1:-1")) {
                            ...
                            // System.out.println("Desaparece " + i + ": " + instant + " ultima Notificacion "
    ...
    private int compararPuntos(
            final Point oldPoint, final Point newPoint, final int id) throws IOException {
        ...
        if ((oldPoint.getX() == -1) && (oldPoint.getY() == -1)) {
            ...
            if (!this.mapIdsDesaparecidos.get(id).equals("-1:-1")) {
                ...
                // System.out.println("Reaparece " + id + ": " + newPoint.getInstant());
            }
            return 1;
        } else {
            ...
            if ((deltaX < MAXIMO_DESPLAZAMIENTO) && (deltaY < MAXIMO_DESPLAZAMIENTO)) {
                ...
                // String key = (newPoint.getX() - oldPoint.getX()) + ":" + (newPoint.getY() - oldPoint.getY());
                // if (mapFrecuency.containsKey(key)) {
                // mapFrecuency.put(key, mapFrecuency.get(key) + 1);
                // } else {
                // mapFrecuency.put(key, 1);
    ...
    private void escribirResultados() throws IOException {
        ...
        // statiticswriter.write("Datos: " + System.getProperty("line.separator"));
        // statiticswriter.write("Tamaño celda: " + "1,41 m2 * " + TAMANO_CELDAS +
        // System.getProperty("line.separator"));
        // statiticswriter.write("Frecuencia de muestreo: " + FREQ_MUESTREO + " segundos" +
        // System.getProperty("line.separator"));
        // statiticswriter.write("Nº medio de objetos en estado desconocido: " + df.format((float)
        // numObjetosDesconocidos / momentos) + System.getProperty("line.separator"));
        ...
        System.out.println("Nº medio de objetos en estado desconocido: "
                + df.format((float) numObjetosDesconocidos / momentos));
    ...
    private int unidimensionar(
        ...
        if ((key != "reaparición") && (key != "desaparición")) {
            ...
            // max2 siempre sera par por lo que se tratará de la esquina inferior izquierda de la espiral
            if ((x == -max) || (y == -max)) {
            ...
            // Puntos inalcanzables por los barcos pero que si que entran dentro de los puntos posibles
    ...
    private void analizadorDeLimites(final Double menorLatitud, final Double mayorLatitud, final Double menorLongitud,
            final Double mayorLongitud, final Set<Integer> idsObjetos, final long numeroLineas, final int ultimoInstante) {
        double ladoSuperior = ControladorHelper.haversine_km(mayorLatitud, menorLongitud, mayorLatitud,
                mayorLongitud);
        double ladoInferior = ControladorHelper.haversine_km(menorLatitud, menorLongitud, menorLatitud,
                mayorLongitud);
        ...
        final double ladoLateral = ControladorHelper.haversine_km(menorLatitud, mayorLongitud, mayorLatitud,
                mayorLongitud);
        final int numeroCeldasLado = (int) Math.ceil((Math.max(ladoLateral, ladoSuperior) * 1000)
