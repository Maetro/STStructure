...
public class PreparadorDatos {
		...
		for (int i = 1; i <= this.numeroObjetos; i++) {
			if ((FILTRADO && (ID_FILTRADO == i)) || !FILTRADO) {
				...
				if ((punto.getX() != -1) && (punto.getY() != -1)) {
					...
					mainWriter.write(lastInstant + " " + i + " " + -1 + " " + -1 + System.getProperty("line.separator"));
				}
			}
		}
		momentos++;
	...
	/**
	 * En el primer analisis se establecen los limites del mapa y el numero de
	 * objetos diferentes que existen
	 */
	...
	private void crearFicheroNormalizado() {
		final Resource ficheroEntrada = this.resourceLoader.getResource("classpath:imis1day");
		final Resource ficheroSalida = this.resourceLoader.getResource("classpath:imis1dayNormalizado");
		// Inicializamos el lector del fichero
		InputStream is;
		...
		try {
			...
			final BufferedReader br = new BufferedReader(new InputStreamReader(is));
			final BufferedWriter writer = new BufferedWriter(new FileWriter(ficheroSalida.getFile()));
			String line;
			final ParseadorFicherosEntrada parseador = new ParseadorFicherosEntradaImis(
					this.configuracion.getSegundosEntreInstantes());
			...
			while ((line = br.readLine()) != null) {
				...
				String lineaActual = datos.getInstante() + " " + datos.getIdObjeto() + " "
						+ normalizador.calcularXnormalizado(datos.getPosX()) + " "
						+ normalizador.calcularYnormalizado(datos.getPosY());
	...
	private void analizadorDeLimites(final Double menorLatitud, final Double mayorLatitud, final Double menorLongitud,
		...
		normalizador = new Normalizador(menorLatitud, mayorLatitud, menorLongitudCorregida, mayorLongitudCorregida,
				numeroCeldasLado);
